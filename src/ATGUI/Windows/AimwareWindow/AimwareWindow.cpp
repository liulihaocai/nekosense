...
std::initializer_list<const char *> subtabVisuals = {"Players", "Radar", "Hitmarker", "Material", "Others"};
...
//std::initializer_list<const char *> subtabColorPanel = {"ESP", "Chams", "Grenade", "Others"};
...
static void BgDecorations()
{
	...
	//
	//
	draw->AddText(info, 13.f, ImVec2(pos.x + 12, pos.y + 625), ImColor(255, 255, 255), "1.1 Beta");
	draw->AddText(info, 13.f, ImVec2(pos.x + 100, pos.y + 625), ImColor(255, 255, 255),
	              "Counter-Strike: Global Offensive");
	draw->AddText(info, 13.f, ImVec2(pos.x + 860, pos.y + 625), ImColor(255, 255, 255), "Kali.cc");
	//
	// Side Panel Bg
	...
	if (ImGui::Begin(XORSTR("##MissedIt-AimWare-Menu"), &Main::showWindow,
		...
		switch (tabs) {
			...
			case MiscTab:
				...
				break;
			//case ColorsTab:
				//AWcolor::RenderAimware(pos, draw, subtabs);
				//break;
			case Config:
