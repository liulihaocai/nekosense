...
static void ThirdPersonSection()
{
	...
	ImGui::PushItemWidth(-1);
	ImGui::SliderFloat(XORSTR("##CameraOffset"), &Settings::ThirdPerson::distance, 0.f, 500.f,
...
void Misc::RenderTab()
{
	ImGui::Columns(2, nullptr, false);
	{
		ImGui::BeginChild(XORSTR("Child1"), ImVec2(0, 0), false);
		{
			...
			ImGui::BeginGroupPanel(XORSTR("Field of view"));
			{
				FovSection();
			}
			ImGui::EndGroupPanel();
...
void Misc::RenderAimware(ImVec2 &pos, ImDrawList *draw, int sideTabIndex)
{
	...
	ImGui::BeginGroup();
	{
		if (sideTabIndex == BunnyHop) {
			...
			Kali::Destroy(); // destroy self and unload itself
		} else { //  others
			ImGui::Columns(2, nullptr, false);
			{
				ImGui::BeginChild(XORSTR("Child3"), ImVec2(0, 0), false);
