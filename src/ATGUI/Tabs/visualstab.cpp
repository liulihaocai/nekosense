...
static const char *BarTypes[] = {"Vertical Left", "Vertical Right", "Horizontal Below", "Horizontal Above",
                                 "Interwebz"};
...
static void FilterEnemies()
{
	...
	ImGui::SameLine();
	ImGui::Combo(XORSTR("##TRACERTYPEEnemy"), (int *) &Settings::ESP::FilterEnemy::Tracers::type, TracerTypes,
...
static void FilterLocalPlayer()
{
	...
	ImGui::Text(XORSTR("Tracers"));
	ImGui::SameLine();
	ImGui::Combo(XORSTR("##TRACERTYPELocal"), (int *) &Settings::ESP::FilterLocalPlayer::Tracers::type, TracerTypes,
	             IM_ARRAYSIZE(TracerTypes));
...
static void FilterAlice()
{
	...
	ImGui::Text(XORSTR("Tracers"));
	ImGui::SameLine();
	ImGui::Combo(XORSTR("##TRACERTYPEAlise"), (int *) &Settings::ESP::FilterAlice::Tracers::type, TracerTypes,
	             IM_ARRAYSIZE(TracerTypes));
...
void Visuals::Others()
{
	...
	ImGui::SameLine();
	ImGui::Combo(XORSTR("##WEAPONTYPE"), (int *) &Settings::ESP::Chams::Weapon::type, chamsTypes,
	             IM_ARRAYSIZE(chamsTypes));
	...
	ImGui::Columns(2, nullptr, false);
	{
		ImGui::BeginChild(XORSTR("##Visuals1"), ImVec2(-1, -1), false);
		{
			...
			if (ImGui::BeginCombo(XORSTR("##CrosshairOptions"), XORSTR("Crosshair Options"))) {
				ImGui::Columns(2, nullptr, false);
				{
					ImGui::Selectable(XORSTR("Recoil Crosshair"), &Settings::Recoilcrosshair::enabled,
					                  ImGuiSelectableFlags_DontClosePopups);
					ImGui::Selectable(XORSTR("FOV Circle"), &Settings::ESP::FOVCrosshair::enabled,
					                  ImGuiSelectableFlags_DontClosePopups);
					ImGui::Selectable(XORSTR("Show Spread"), &Settings::ESP::Spread::enabled,
					                  ImGuiSelectableFlags_DontClosePopups);
				}
				ImGui::NextColumn();
				{
					ImGui::Selectable(XORSTR("Only When Shooting"), &Settings::Recoilcrosshair::showOnlyWhenShooting,
					                  ImGuiSelectableFlags_DontClosePopups);
					ImGui::Selectable(XORSTR("Filled"), &Settings::ESP::FOVCrosshair::filled,
					                  ImGuiSelectableFlags_DontClosePopups);
					ImGui::Selectable(XORSTR("Show SpreadLimit"), &Settings::ESP::Spread::spreadLimit,
					                  ImGuiSelectableFlags_DontClosePopups);
				}
				ImGui::EndCombo();
			}
...
void MaterialTab::MaterialConfigButtons()
{
	static bool localFlags[] = {
			(Settings::MaterialConfig::config.m_Flags &
			MaterialSystem_Config_Flags_t::MATSYS_VIDCFG_FLAGS_WINDOWED) != 0, // ( 1 << 0 )
...
void MaterialTab::MaterialConfigTab()
{
ImGui::CheckboxFill(XORSTR("EditMode"), &Settings::MaterialConfig::config.bEditMode);
//proxiesTestMode
ImGui::CheckboxFill(XORSTR ("Compressed Textures"),
...
void MaterialTab::MaterialSecondTab()
{
ImGui::CheckboxFill(XORSTR("Shadow Depth Texture"),
	                 &Settings::MaterialConfig::config.m_bShadowDepthTexture);
SetTooltip(XORSTR("Risky. May cause black Screen. Reset if it does."));
...
void MaterialTab::MaterialAdjustable()
{
ImGui::SliderFloat(XORSTR("##MONITORGAMMA"), &Settings::MaterialConfig::config.m_fMonitorGamma, 0.1f,
				      12.0f,
				      XORSTR("Gamma: %.3f"));
...
void MaterialTab::MaterialDisplay()
{
	ImGui::Text(XORSTR("Width:"));
	ImGui::SameLine();
	ImGui::InputInt(XORSTR("##GAMEWIDTH"), &Settings::MaterialConfig::config.m_VideoMode.m_Width);
...
void MaterialTab::MaterialAntiAliasing()
{
ImGui::CheckboxFill(XORSTR("TripleBuffered"), &Settings::MaterialConfig::config.m_bTripleBuffered);	
ImGui::SliderInt(XORSTR("##AASAMPLES"), &Settings::MaterialConfig::config.m_nAASamples, 0, 16,
			       XORSTR("AA Samples: %1.f"));
...
void MaterialTab::MaterialConfig()
{
	static bool localFlags[] = {
			(Settings::MaterialConfig::config.m_Flags &
			MaterialSystem_Config_Flags_t::MATSYS_VIDCFG_FLAGS_WINDOWED) != 0, // ( 1 << 0 )
...
void Visuals::RenderAimware(ImVec2 &pos, ImDrawList *draw, int sideTabIndex)
{
	...
	} else if (sideTabIndex == Material_Config) {
		ImGui::BeginGroup();
		{
			ImGui::Columns(3, nullptr, false);
			{
				ImGui::BeginChild(XORSTR("##PlayerVisuals1"), ImVec2(0, 0), false);
				{
					ImGui::BeginGroupPanel(XORSTR("Display Configuration"));
					{
						MaterialTab::MaterialDisplay();
					}
					ImGui::EndGroupPanel();

					ImGui::BeginGroupPanel(XORSTR("Material Config"));
					{
						MaterialTab::MaterialConfig();
					}
					ImGui::EndGroupPanel();
				}
				ImGui::EndChild();
			...
			ImGui::NextColumn();
			{
				ImGui::BeginChild(XORSTR("##PlayerVisuals2"), ImVec2(0, 0), false);
				{
					ImGui::BeginGroupPanel(XORSTR("Customizations"));
					{
						MaterialTab::MaterialConfigTab();
					}
					ImGui::EndGroupPanel();
			...
			ImGui::NextColumn();
			{
				ImGui::BeginChild(XORSTR("##PlayerVisuals3"), ImVec2(0, 0), false);
				{
					ImGui::BeginGroupPanel(XORSTR("Adjustable Display Settings"));
					{
						MaterialTab::MaterialAdjustable();
					}
					ImGui::EndGroupPanel();
				ImGui::BeginGroupPanel(XORSTR("Options"));
					{
						MaterialTab::MaterialSecondTab();
					}
					ImGui::EndGroupPanel();
				ImGui::BeginGroupPanel(XORSTR("Customize AntiAliasing"));
					{
						MaterialTab::MaterialAntiAliasing();
					}
					ImGui::EndGroupPanel();

				ImGui::BeginGroupPanel(XORSTR("Buttons"));
					{
						MaterialTab::MaterialConfigButtons();
					}
					ImGui::EndGroupPanel();
