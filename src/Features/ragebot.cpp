...
#define RandomeFloat(x)                                                        \
  (static_cast<double>(static_cast<double>(std::rand()) /                      \
                       static_cast<double>(RAND_MAX / x)))
...
std::vector<long> RagebotkillTimes = {
    0}; // the Epoch time from when we kill someone
...
float get_interpolation() {
  ...
  static auto sv_client_min_interp_ratio =
      cvar->FindVar("sv_client_min_interp_ratio");
  static auto sv_client_max_interp_ratio =
      cvar->FindVar("sv_client_max_interp_ratio");
  ...
  auto updaterate =
      clamp(cl_updaterate->GetFloat(), sv_minupdaterate->GetFloat(),
            sv_maxupdaterate->GetFloat());
  auto lerp_ratio =
      clamp(cl_interp_ratio->GetFloat(), sv_client_min_interp_ratio->GetFloat(),
            sv_client_max_interp_ratio->GetFloat());

  return clamp(lerp_ratio / updaterate, cl_interp->GetFloat(), 1.0f);
}

void GetDamageAndSpots(C_BasePlayer *player, Vector &Spot, int &Damage,
                       int &playerHealth, int &i,
                       const std::unordered_map<int, int> *modelType,
                       const RageWeapon_t &currentSetting) {
  ...
  static auto HitboxHead([&](int BoneID) {
    ...
    Ragebot::ragebotPredictionSystem->BestHeadPoint(player, BoneID, Damage,
                                                    Spot);
  });
  static auto UpperSpine([&](int BoneID) {
    Spot = player->GetBonePosition(BoneID);
    Ragebot::ragebotPredictionSystem->BestMultiPoint(player, BoneID, Damage,
                                                     Spot);
    if (Damage >= playerHealth)
      return;

    static const int BONE[] = {BONE_LEFT_COLLARBONE, BONE_RIGHT_COLLARBONE,
                               BONE_LEFT_SHOULDER, BONE_RIGHT_SHOULDER};
  ...
  static auto MiddleSpine([&](int BoneID) {
    ...
    static const int BONE[] = {BONE_LEFT_ARMPIT, BONE_RIGHT_ARMPIT,
                               BONE_LEFT_BICEP,  BONE_RIGHT_BICEP,
                               BONE_LEFT_ELBOW,  BONE_RIGHT_ELBOW};
  ...
  static auto PelvisHitbox([&](int BoneID) {
    ...
    static const int BONE[] = {BONE_LEFT_KNEE,   BONE_LEFT_ANKLE,
                               BONE_LEFT_SOLE,   BONE_RIGHT_BUTTCHEEK,
                               BONE_RIGHT_THIGH, BONE_RIGHT_KNEE,
                               BONE_RIGHT_ANKLE, BONE_RIGHT_SOLE};
  ...
  case DesireBones::BONE_HEAD:
    ...
    if (currentSetting.desiredMultiBones[i])
      HitboxHead(boneID); // lamda expression because again creating a new
                          // method is going to make the source code mess :p
    else
...
void GetBestSpotAndDamage(C_BasePlayer *player, C_BasePlayer *localplayer,
                          Vector &Spot, int &Damage,
                          const RageWeapon_t &currSettings) {
  if (!player || !localplayer || !player->GetAlive() ||
  ...
  const std::unordered_map<int, int> *modelType =
      BoneMaps::GetModelTypeBoneMap(player);
  ...
  static int i;
  for (i = 0; i < 6; i++) {
    GetDamageAndSpots(player, spot, damage, playerHealth, i, modelType,
                      currSettings);
    if (damage >= playerHealth) {
      if (!Settings::Ragebot::onshot::enabled ||
          !(inputSystem->IsButtonDown(Settings::Ragebot::onshot::button))) {
        Damage = damage;
        Spot = spot;
        return;
      } else if (Settings::Ragebot::onshot::enabled &&
                 inputSystem->IsButtonDown(Settings::Ragebot::onshot::button)) {
        CUtlVector<AnimationLayer> *layers = player->GetAnimOverlay();
        if (player->GetSequenceActivity(layers->operator[](1).m_nSequence) ==
            (int)CCSGOAnimStatePoses::ACT_CSGO_FIRE_PRIMARY) {
          Damage = damage;
          Spot = spot;
          return;
        } else
...
void RagebotNoRecoil(QAngle &angle, CUserCmd *cmd, C_BasePlayer *localplayer,
...
void RagebotAutoCrouch(C_BasePlayer *localplayer, CUserCmd *cmd,
                       C_BaseCombatWeapon *activeWeapon,
                       const RageWeapon_t &currentSettings) {
  if (!localplayer || !localplayer->GetAlive() ||
      !Settings::Ragebot::AutoCrouch::enabled)
    return;
...
void RagebotAutoSlow(C_BasePlayer *localplayer,
                     C_BaseCombatWeapon *activeWeapon, CUserCmd *cmd,
                     float &forrwordMove, float &sideMove, QAngle &angle,
                     const RageWeapon_t &currentSettings) {
  ...
  if (currentSettings.autoScopeEnabled &&
      ...
      !localplayer->IsScoped() && !(cmd->buttons & IN_ATTACK2) &&
...
void Ragebot::CheckHit(C_BaseCombatWeapon *activeWeapon) {
  ...
  if (!Ragebot::lockedEnemy.shooted || !Ragebot::lockedEnemy.player)
    return;
  // Util::ConsoleLog("2 \n");
  if (Ragebot::lockedEnemy.playerhelth !=
          Ragebot::lockedEnemy.player->GetHealth() ||
      !Ragebot::lockedEnemy.player->GetAlive())
    return;
  // Util::ConsoleLog("3 \n");
  // if ( activeWeapon->GetNextPrimaryAttack() >
  // static_cast<float>(localplayer->GetTickBase()) * TICK_INTERVAL)
  //      return;
...
// traceStart = localplayer->GetEyePosition();
  ...
  // Util::ConsoleLog("traced \n");
  if (Ragebot::lockedEnemy.playerhelth == lockedEnemy.player->GetHealth() &&
      lockedEnemy.player->GetAlive()) {
    if (tr.m_pEntityHit == lockedEnemy.player || spred > 0.003300) {
      Resolver::players[Ragebot::lockedEnemy.player->GetIndex()].MissedCount++;

      Util::ConsoleLog("[Kali] Missed shot due to bad resolve [RESOLVER: ");
      switch (Settings::Resolver::resolverType) {
      case resolverType::Experimental:
        if (!Settings::Resolver::manual) {
          Util::ConsoleLog("EXP, STEP ");
          Util::ConsoleLog(
              std::to_string(Resolver::players[Resolver::TargetID].MissedCount)
                  .c_str());
        } else if (!Settings::Resolver::forcebrute) {
          Util::ConsoleLog("MANUAL, Y: ");
          Util::ConsoleLog(
              std::to_string(Settings::Resolver::EyeAngles).c_str());
          Util::ConsoleLog(" , X: ");
          Util::ConsoleLog(std::to_string(Settings::Resolver::Pitch).c_str());
        } else if (Settings::Resolver::forcebrute &&
                   Settings::Resolver::manual) {
          Util::ConsoleLog("BRUTEFORCE");
        }
        break;
      case resolverType::ApuWare:
        Util::ConsoleLog("ApuWare");
      ...
      Util::ConsoleLog("[Kali] Missed shot due to spread", true);
    }
  }
  if (Resolver::players[Ragebot::lockedEnemy.player->GetIndex()].MissedCount >
      4) {
...
void RagebotAutoR8(C_BasePlayer *player, C_BasePlayer *localplayer,
                   C_BaseCombatWeapon *activeWeapon, CUserCmd *cmd,
                   Vector &bestspot, QAngle &angle, float &forrwordMove,
                   float &sideMove, const RageWeapon_t &currentSettings) {
...
void RagebotAutoShoot(C_BasePlayer *player, C_BasePlayer *localplayer,
                      C_BaseCombatWeapon *activeWeapon, CUserCmd *cmd,
                      Vector &bestspot, QAngle &angle, float &forrwordMove,
                      float &sideMove, const RageWeapon_t &currentSettings) {
  if (!currentSettings.autoShootEnabled)
  ...
  if (*activeWeapon->GetItemDefinitionIndex() ==
      ItemDefinitionIndex::WEAPON_REVOLVER)
    return;
  if (Ragebot::ragebotPredictionSystem->canShoot(
          cmd, localplayer, activeWeapon, bestspot, player, currentSettings)) {
    if (activeWeapon->GetNextPrimaryAttack() > globalVars->curtime)
      cmd->buttons &= ~IN_ATTACK;
...
void Ragebot::gotoStart(CUserCmd *cmd) {
  ...
  auto velocity =
      Vector(difference.x * cos(cmd->viewangles.y / 180.0f * M_PI) +
                 difference.y * sin(cmd->viewangles.y / 180.0f * M_PI),
             difference.y * cos(cmd->viewangles.y / 180.0f * M_PI) -
                 difference.x * sin(cmd->viewangles.y / 180.0f * M_PI),
             difference.z);
  ...
  //      (float)(VecForward.x * cos(yaw / 180 * (float)M_PI) + VecForward.y *
  //      sin(yaw / 180 * (float)M_PI)),
  //     (float)(VecForward.y * cos(yaw / 180 * (float)M_PI) - VecForward.x *
  //     sin(yaw / 180 * (float)M_PI)),
  //      VecForward.z
  //  };
...
void Ragebot::drawStartPos() {
  ...
  if (quickpeekstartpos != Vector{0, 0, 0}) {
    ...
    Draw::AddLine(playerPos.x, playerPos.y, spot2D.x, spot2D.y,
                  Settings::Ragebot::quickpeek::color.Color());

    Draw::FilledCircle3D(
        quickpeekstartpos, 32, 32,
        Color::FromImColor(Settings::Ragebot::quickpeek::color.Color()));
  }
}
static void FixMouseDeltas(CUserCmd *cmd, C_BasePlayer *player, QAngle &angle,
                           QAngle &oldAngle) {
...
C_BasePlayer *GetBestEnemyAndSpot(C_BasePlayer *localplayer,
                                  const RageWeapon_t &currSettings) {
  ...
  if (lockedEnemy.player) {
    if (lockedEnemy.player->GetAlive() && !lockedEnemy.player->GetDormant() &&
        !Ragebot::lockedEnemy.player->GetImmune()) {
      // studiohdr_t* pStudioModel = modelInfo->GetStudioModel(
      // lockedEnemy.player->GetModel() ); if ( pStudioModel ) bestSpot =
      // Vector(
      // Resolver::players[lockedEnemy.player->GetIndex()].ogmatrix[Resolver::players[lockedEnemy.player->GetIndex()].safepoints][0][3],
      // Resolver::players[lockedEnemy.player->GetIndex()].ogmatrix[Resolver::players[lockedEnemy.player->GetIndex()].safepoints][1][3],
      // Resolver::players[lockedEnemy.player->GetIndex()].ogmatrix[Resolver::players[lockedEnemy.player->GetIndex()].safepoints][2][3]);
      GetBestSpotAndDamage(lockedEnemy.player, localplayer, bestSpot,
                           bestDamage, currSettings);
      if (bestDamage >= lockedEnemy.player->GetHealth() ||
          bestDamage >= currSettings.MinDamage ||
          (Settings::Ragebot::mindmgoverride &&
           inputSystem->IsButtonDown(Settings::Ragebot::dmgkey))) {
  ...
  for (int i = 1; i < maxClient; ++i) {
    ...
    if (Entity::IsTeamMate(player,
                           localplayer)) // Checking for Friend If any it will
...
void Ragebot::CreateMove(CUserCmd *cmd) {
  ...
  if (Settings::Ragebot::impacttype == impactType::ITSME) {
    if (Ragebot::lockedEnemy.shooted && lockedEnemy.player &&
        activeWeapon->GetNextPrimaryAttack() <=
            static_cast<float>(localplayer->GetTickBase()) * TICK_INTERVAL &&
        localplayer->GetAlive() && !(activeWeapon->GetInReload())) {
      ...
      if (Ragebot::lockedEnemy.playerhelth == lockedEnemy.player->GetHealth() &&
        ...
        float spred = activeWeapon->GetSpread() + activeWeapon->GetInaccuracy();
        Ragebot::miss = true;
        if ((tr.m_pEntityHit == lockedEnemy.player || spred < 0.003300) && (Settings::Resolver::
                                                                                resolveAll /*&&Resolver::players[lockedEnemy.player->GetIndex()].flags != Resolver::rflag::NONE*/)) // We arent gonna miss due to spread while standing still scoped in and crouched ;-;
        {
          Resolver::players[Resolver::TargetID].MissedCount++;
          Util::ConsoleLog("[Kali] Missed shot due to bad resolve [RESOLVER: ");
          Ragebot::misstring =
              "Missed shot due to bad resolve [RESOLVER: "; // Shit way of doing
                                                            // this but im lazy.
          switch (Settings::Resolver::resolverType) {
          case resolverType::Experimental:
            if (!Settings::Resolver::manual) {
              ...
              Ragebot::misstring += std::to_string(
                  Resolver::players[Resolver::TargetID].MissedCount);
              Util::ConsoleLog(
                  std::to_string(
                      Resolver::players[Resolver::TargetID].MissedCount)
                      .c_str());
              Util::ConsoleLog(" , FLAGS: ");
              if (Resolver::players[lockedEnemy.player->GetIndex()].flags ==
                  Resolver::rflag::LAA)
                Util::ConsoleLog("LegitAA");

              else if (Resolver::players[lockedEnemy.player->GetIndex()]
                           .flags == Resolver::rflag::LOW)
                Util::ConsoleLog("Low");

              else if (Resolver::players[lockedEnemy.player->GetIndex()]
                           .flags == Resolver::rflag::LBY)
                Util::ConsoleLog("LBY");
            }
              ...
              Util::ConsoleLog(" , X: ");
              Util::ConsoleLog(
                  std::to_string(Settings::Resolver::Pitch).c_str());
            } else if (Settings::Resolver::forcebrute &&
                       Settings::Resolver::manual) {
              Util::ConsoleLog("BRUTEFORCE");
            }
          ...
          Util::ConsoleLog("[Kali] Missed shot due to spread / ragebot", true);
  ...
  if (Settings::Ragebot::weapons.find(
          *activeWeapon->GetItemDefinitionIndex()) !=
      Settings::Ragebot::weapons.end())
    index = *activeWeapon->GetItemDefinitionIndex();
  const RageWeapon_t &currentRageWeaponSetting =
      Settings::Ragebot::weapons.at(index);
  ...
  RagebotAutoR8(player, localplayer, activeWeapon, cmd, Ragebot::BestSpot,
                angle, oldForward, oldSideMove, currentRageWeaponSetting);
  if (player)
    cmd->tick_count =
        TIME_TO_TICKS(player->GetSimulationTime() + get_interpolation());
  bool FD = (Settings::FakeDuck::enabled &&
             inputSystem->IsButtonDown(Settings::FakeDuck::key) &&
             cmd->buttons & IN_DUCK);
  if (player && Ragebot::BestDamage > 0 && !FD) {
    ...
    RagebotAutoShoot(player, localplayer, activeWeapon, cmd, Ragebot::BestSpot,
                     angle, oldForward, oldSideMove, currentRageWeaponSetting);
...
void Ragebot::FireGameEvent(IGameEvent *event) {
  ...
  if (strcmp(event->GetName(), XORSTR("bullet_impact")) == 0 &&
      engine->GetPlayerForUserID(event->GetInt(XORSTR("userid"))) ==
          engine->GetLocalPlayer()) {
    float x = event->GetFloat(XORSTR("x")), y = event->GetFloat(XORSTR("y")),
...
void Ragebot::UpdateValues() {
  ...
  Settings::Ragebot::AutoPistol::enabled =
      currentRageWeaponSetting.autoPistolEnabled;
  Settings::Ragebot::AutoShoot::enabled =
      currentRageWeaponSetting.autoShootEnabled;
  Settings::Ragebot::AutoShoot::autoscope =
      currentRageWeaponSetting.autoScopeEnabled;
  Settings::Ragebot::HitChance::enabled =
      currentRageWeaponSetting.HitChanceEnabled;
  ...
  Settings::Ragebot::ScopeControl::enabled =
      currentRageWeaponSetting.scopeControlEnabled;
  Settings::Ragebot::HitChance::value = currentRageWeaponSetting.HitChance;
  Settings::Ragebot::damagePrediction =
      currentRageWeaponSetting.DmagePredictionType;
  Settings::Ragebot::enemySelectionType =
      currentRageWeaponSetting.enemySelectionType;
  ...
  for (int bone = 0; bone < 6; bone++) {
    Settings::Ragebot::AutoAim::desireBones[bone] =
        currentRageWeaponSetting.desireBones[bone];
    Settings::Ragebot::AutoAim::desiredMultiBones[bone] =
        currentRageWeaponSetting.desiredMultiBones[bone];
  }
}