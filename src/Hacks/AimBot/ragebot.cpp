...
void Ragebot::AutoShoot(C_BasePlayer *enemy, CUserCmd *cmd, QAngle angle, RageWeapon_t *currentSettings)
{
	...
	if (currentSettings->autoScopeEnabled &&
	    Util::Items::IsScopeable(*Ragebot::activeWeapon->GetItemDefinitionIndex()) &&
	    !Ragebot::localplayer->IsScoped() && !(cmd->buttons & IN_ATTACK2) && !(cmd->buttons & IN_ATTACK)) {
                                ...
                                const std::unordered_map<int, int> *modelType, matrix3x4_t bonematrix[])
{
	...
	switch (static_cast<DesireBones>(i)) {
		case DesireBones::BONE_HEAD:
			...
			if (Ragebot::currentWeaponSetting->desiredMultiBones[i]) {
				HitboxHead(
...
void Ragebot::CheckHit()
{
	...
	while (fireBulletData.penetrate_count > 0 && fireBulletData.current_damage >= 1.0f) {
		cvar->ConsoleColorPrintf(ColorRGBA(254, 0, 0, 255), XORSTR("[Kali] round \n"));
		...
		if (fireBulletData.enter_trace.hitgroup < HitGroups::HITGROUP_GEAR &&
		    fireBulletData.enter_trace.hitgroup > HitGroups::HITGROUP_GENERIC) {
			cvar->ConsoleColorPrintf(ColorRGBA(254, 0, 0, 255), XORSTR("[Kali] Hit\n"));
	...
	cvar->ConsoleColorPrintf(ColorRGBA(254, 0, 0, 255), XORSTR("[Kali] Miss Due To Spread\n"));
}
...
void Ragebot::CreateMove(CUserCmd *cmd)
{
	if (Ragebot::data.player && Ragebot::data.player->IsAlive()) {
		if (Ragebot::data.Hitted && Ragebot::data.player->GetHealth() >= Ragebot::data.playerhealth) {
			cvar->ConsoleColorPrintf(ColorRGBA(255, 0, 0, 255), XORSTR("[Kali] Miss Due To Resolver\n"));
	...
	Ragebot::activeWeapon = (C_BaseCombatWeapon *) entityList->GetClientEntityFromHandle(
			Ragebot::localplayer->GetActiveWeapon());
	if (!Ragebot::activeWeapon || Ragebot::activeWeapon->GetInReload() || Ragebot::activeWeapon->GetAmmo() == 0) {
		return;
	}
...
void Ragebot::FireGameEvent2(IGameEvent *event)
{
	...
	} else if (strcmp(event->GetName(), XORSTR("player_connect_full")) == 0 ||
