...
std::vector<Config> GetConfigs(const char *directory) {
  ...
  while ((pdir = readdir(dir))) {
    if (pdir->d_type != DT_DIR || strcmp(pdir->d_name, ".") == 0 ||
        ...
        strcmp(pdir->d_name, "!scripts") == 0 ||
        strcmp(pdir->d_name, "!gh") == 0)
      continue;
...
std::string GetGhConfigDirectory() {
  std::ostringstream directory;
  directory << GetConfigDirectory() << XORSTR("!gh/");
...
std::string GetScriptDirectory() {
  std::ostringstream directory;
  directory << GetConfigDirectory() << XORSTR("!scripts/");
